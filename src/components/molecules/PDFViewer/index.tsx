...
const getDeclationData = (data: any) => {
  if (!data) {
    return {};
  }
  ...
  if (
    !(
      data.bf_data_process_personasjuridicas &&
      Array.isArray(data.bf_data_process_personasjuridicas) &&
      data.bf_data_process_personasjuridicas.length > 0
    )
  ) {
    return {};
  }
  const person_data = data.bf_data_process_personasjuridicas[0];
  return {
    ...person_data,
    rut_no: person_data.rut,
    fechahora_creacion: formatDate(data.fecha_declaracion),
    tipo_sociedad: person_data.tipo_de_sociedad,
  };
};
...
export const PDFTableInfo = ({ declaracion }: { declaracion: PJuridicas }) => {
  ...
  } = getDeclationData(declaracion);
  const rutRep = rut;
  return (
    <View style={stylesInfo.container}>
      <View style={stylesInfo.tableColumn}>
        <Text style={stylesInfo.tableColumnTitle}>RUT/N Identificación</Text>
        <Text style={{ fontSize: "10px" }}>{rut}</Text>
      </View>
      <View style={stylesInfo.tableColumn}>
        <Text style={stylesInfo.tableColumnTitle}>Domicilio</Text>
        <Text style={{ fontSize: "10px" }}>{domicilio}</Text>
      </View>
      <View style={stylesInfo.tableColumn}>
        <Text style={stylesInfo.tableColumnTitle}>Telefono</Text>
        <Text style={{ fontSize: "10px" }}>{telefono}</Text>
      </View>
      <View style={stylesInfo.tableColumn}>
        <Text style={stylesInfo.tableColumnTitle}>Ciudad</Text>
        <Text style={{ fontSize: "10px" }}>{ciudad}</Text>
      </View>
      <View style={stylesInfo.tableColumn}>
        <Text style={stylesInfo.tableColumnTitle}>Constitucion</Text>
        <Text style={{ fontSize: "10px" }}>{constitucion}</Text>
      </View>
      <View style={stylesInfo.tableColumn}>
        <Text style={stylesInfo.tableColumnTitle}>Nombre Rep. Legal</Text>
        <Text style={{ fontSize: "10px" }}>{nombreRep}</Text>
      </View>
      <View style={stylesInfo.tableColumn}>
        <Text style={stylesInfo.tableColumnTitle}>RUT Rep. Legal</Text>
        <Text style={{ fontSize: "10px" }}>{rutRep}</Text>
      </View>
      <View style={stylesInfo.tableColumn}>
        <Text style={stylesInfo.tableColumnTitle}>Tipo de Sociedad</Text>
        ...
        registros.map((registro: any, index: any) => (
          <View key={index} style={stylesTable.tableRow}>
            <View style={{ flexDirection: "row" }}>
              ...
              <View style={{ flexDirection: "column", flex: 1 }}>
                <Text style={stylesTable.tableColumnTitle}>Participación</Text>
                <Text style={{ fontSize: "10px" }}>
